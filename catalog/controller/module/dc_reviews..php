...
namespace Opencart\Catalog\Controller\Extension\DcReviews\Module;
class DcReviews extends \Opencart\System\Engine\Controller {
    ...
    public function index(array $setting): string {
        ...
        if (isset($setting['form'][$language_id])) {
            ...
            $this->document->addStyle(HTTP_SERVER . 'extension/dc_reviews/catalog/view/javascript/module-dc_reviews/dc_reviews.css');
            ...
            if ($data['datetimepicker'] = $this->datetimepicker) {
                $this->document->addStyle(HTTP_SERVER . 'extension/dc_reviews/catalog/view/javascript/module-dc_reviews/datetimepicker-1.3.6/jquery.datetimepicker.min.css');
                $this->document->addScript(HTTP_SERVER . 'extension/dc_reviews/catalog/view/javascript/module-dc_reviews/datetimepicker-1.3.6/jquery.datetimepicker.full.min.js');
            }
            ...
            $fcode .= $setting['mcode'] . '---';
            $fcode .= $this->config->get('module_dc_reviews_captcha_ed_ec');

            $data['fcode'] = openssl_encrypt($fcode, 'AES-128-ECB', $this->config->get('module_dc_reviews_captcha_ed_pc'));

            $data['action'] = $this->url->link('extension/dc_reviews/module/dc_reviews' . $this->x . 'submit', '');

            $data['module'] = $module++;

            $view = $this->load->view('extension/dc_reviews/module/dc_reviews', $data);
        }
    ...
    public function submit(): void {
        ...
        if (isset($this->request->post['fcode'])) {
            $fcode = openssl_decrypt($this->request->post['fcode'], 'AES-128-ECB', $this->config->get('module_dc_reviews_captcha_ed_pc'));
        } else {
            $fcode = false;
        }
            ...
            if (!empty($fields)) {
                ...
                if (count($uniq_keys) == 4) {
                    ...
                    if (!empty($setting) && isset($setting['uniqid']) && 
                        ...
                        ($uniq_keys[3] == $this->config->get('module_dc_reviews_captcha_ed_ec'))) {
