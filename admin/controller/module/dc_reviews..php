<?php
/**
 * Controller Module DC Reviews Class
 *
 * @version 1.0
 * 
...
namespace Opencart\Admin\Controller\Extension\DcReviews\Module;
class DcReviews extends \Opencart\System\Engine\Controller {
    private $error = array();

    public function index(): void {
        $x = version_compare(VERSION, '4.0.2.0', '>=') ? '.' : '|';

        $this->load->language('extension/dc_reviews/module/dc_reviews');
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            if (!isset($this->request->get['module_id'])) {
                $this->model_setting_module->addModule('dc_reviews.dc_reviews', $this->request->post);
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/dc_reviews/module/dc_reviews', 'user_token=' . $this->session->data['user_token'] . $url)
        );

        $data['action'] = $this->url->link('extension/dc_reviews/module/dc_reviews' . $x . 'save', 'user_token=' . $this->session->data['user_token'] . $url);
        ...
        $this->response->setOutput($this->load->view('extension/dc_reviews/module/dc_reviews', $data));
    }
	...
	public function save(): void {
		...
		if (!$this->user->hasPermission('modify', 'extension/dc_reviews/module/dc_reviews')) {
		...
		if (!$json) {
            ...
            if (!isset($this->request->get['module_id'])) {
                ...
                //$json['redirect'] = $this->url->link('extension/dc_reviews/module/dc_reviews', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id']);
            }
    ...
    */
    public function install(): void {
        ...
        // Add settings.
        $this->model_setting_setting->editSetting('module_dc_reviews', array(
            'module_dc_reviews_captcha_ed_pc' => $this->generateRandomString(8), // Password for encrypt/decrypt module_id settings.
            'module_dc_reviews_captcha_ed_ec' => $this->generateRandomString(8)  // Secure string for encrypt/decrypt module_id settings.
        ));
    }
    ...
    */
    public function uninstall(): void {
        $this->load->model('setting/setting');
        $this->model_setting_setting->deleteSetting('module_dc_reviews');
    }
